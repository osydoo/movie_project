	...
	if (m_p->actors != NULL) {
		...
		if (root_a_num == 0) { //배우 레코드에 아무것도 저장되어있지 않을 경우 에러를 피하기 위한 코드
			...
			while (1) {
				...
				if (!strncmp(a_name, a_p->name, (strlen(a_name)>strlen(a_p->name)) ? strlen(a_name) : strlen(a_p->name)))
					break;
				a_p = a_p->next;
			}
        ...
        if (!strncmp(a_name, a_p->name, (strlen(a_name)>strlen(a_p->name)) ? strlen(a_name) : strlen(a_p->name)))
			...
			printf("A%d : %s(%s)\n", i++, a_name==NULL ? "-" : a_name, a_p->next == NULL ? "-" : a_p->birth);
			a_name = strtok(NULL, ",");
		}
	}
	else {
...
void print_d(int sn) { //director 레코드의 print 함수
    ...
    if (!strncmp(a_best_movie, m_p->title, (strlen(a_best_movie)>strlen(m_p->title)) ? strlen(a_best_movie) : strlen(m_p->title))){
			break;
		}
		m_p = m_p->next;
	}
